...
## 웹 화면 버전 원클릭 실행 (setup-webapp.bat + run-webapp.bat, Windows 추천)

아래 "데스크톱 앱(Electron)" 섹션의 설치 파일(.exe)은 **서명되지 않은 실행 파일**이라,
Windows 11의 "스마트 앱 컨트롤"이 켜져 있으면 게시자를 확인할 수 없다며 아예 실행 자체를
차단합니다(유료 코드 서명 인증서가 있어야 우회 가능 - 스마트 앱 컨트롤은 SmartScreen과 달리
"그래도 실행" 버튼이 따로 없습니다). 이 문제를 완전히 피하려면, 새 실행 파일을 만들지 않고
**이미 신뢰된 `python.exe`와 기본 브라우저만으로** 같은 화면을 띄우는 이 방법을 쓰세요.

1. `install.bat`과 동일하게 저장소를 내려받은 뒤, 저장소 루트의 **`setup-webapp.bat`을
   더블클릭**합니다(관리자 권한 필요 없음). Python 확인 → venv/패키지 설치 → ffmpeg 다운로드
   → Node.js 확인 → 웹 화면 빌드까지 자동으로 진행됩니다(처음 한 번만, 몇 분 소요).
2. 완료되면 **`run-webapp.bat`을 더블클릭**합니다. 로컬 서버가 백그라운드 콘솔 창으로 뜨고,
   준비되면 기본 브라우저로 자동으로 화면이 열립니다.
3. 프로그램을 끄려면, 작업 표시줄에 최소화된 "CapCut Auto Editor 서버" 콘솔 창을 닫으면 됩니다.

`setup-webapp.bat`이 끝나면 바탕화면 바로가기(`run-webapp.bat`을 가리킴)를 만들지 물어보므로,
그 이후로는 이 바로가기 더블클릭 → 브라우저 탭 하나로 바로 실행됩니다. Electron 창처럼 별도
타이틀바를 가진 앱은 아니지만 기능은 완전히 동일하며(같은 `capcut_auto/server.py` +
`webapp/dist`를 그대로 씀), 서명되지 않은 실행 파일을 전혀 만들지 않으므로 스마트 앱 컨트롤이
켜져 있어도 막히지 않습니다. 이 방식은 `python -m uvicorn capcut_auto.server:app`을 직접
실행하고 `http://127.0.0.1:8842/`가 200을 응답하는 것까지 이 저장소 안에서 실제로 확인했습니다.
...
**실제 Windows에서 확인된 제약사항**: 이 설치 파일(.exe)은 코드 서명이 안 되어 있어서, Windows
11의 "스마트 앱 컨트롤"이 켜져 있으면 게시자를 검증할 수 없다는 이유로 실행 자체가 차단됩니다
(유료 인증서 없이는 이 기능을 끄는 것 외엔 우회 방법이 없고, 한 번 끄면 재설치 전엔 다시 켤 수
없습니다). 이 문제를 아예 피하고 싶다면 위의 "웹 화면 버전 원클릭 실행" 방법을 대신 쓰세요.
